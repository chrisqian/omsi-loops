...
const townNames = ["新手镇", "Forest Path", "繁商城", "Mt. Olympus", "Valhalla", "Startington", "Jungle Path", "Commerceville", "Valley of Olympus"];
...
class DungeonAction extends MultipartAction {
    ...
    getPartName(loopCounter = towns[this.townNum][`${this.varName}LoopCounter`] + 0.0001) {
        ...
        return `${_txt(`actions>${getXMLName(this.name)}>label_part`)} ${floor <= dungeons[this.dungeonNum].length ? floor+"层" : _txt(`actions>${getXMLName(this.name)}>label_complete`)}`;
    };
}
...
class TrialAction extends MultipartAction {
    ...
    getPartName(loopCounter = towns[this.townNum][`${this.varName}LoopCounter`]) {
        ...
        return `${_txt(`actions>${getXMLName(this.name)}>label_part`)} ${floor <= trials[this.trialNum].length ? floor : _txt(`actions>${getXMLName(this.name)}>label_complete`)}`;
    };
...
Action.HealTheSick = new MultipartAction("Heal The Sick", {
    ...
    getPartName(loopCounter = towns[0].HealLoopCounter) {
        ...
        return `${_txt(`actions>${getXMLName(this.name)}>label_part`)} ${Math.floor((loopCounter + 0.0001) / this.segments + 1)}号`;
    },
    visible() {
        return towns[0].getLevel("Secrets") >= 20;
    },
...
Action.AdventureGuild = new MultipartAction("Adventure Guild", {
    ...
    getPartName() {
        return `${getAdvGuildRank().name}`;
    },
    getSegmentName(segment) {
        return `${getAdvGuildRank(segment % 3).name}`;
    },
    visible() {
...
Action.CraftingGuild = new MultipartAction("Crafting Guild", {
    ...
    getPartName() {
        return `${getCraftGuildRank().name}`;
    },
    getSegmentName(segment) {
        return `${getCraftGuildRank(segment % 3).name}`;
    },
    visible() {
        return towns[2].getLevel("Drunk") >= 5;
    },
