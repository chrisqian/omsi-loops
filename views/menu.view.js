...
const menuView = Views.registerView("menu", {
    ...
    htmlExtrasMenu() {
        const html =
            ...
            <div class='showthisH' style='padding-top:1ex'>
                ...
                <input id='speedIncreaseBackgroundInput' type='number' value='' placeholder='同上' min='0' style='width: 50px;transform: translateY(-2px);' oninput='setOption("speedIncreaseBackground", parseFloat(this.value))' />
    ...
    totals() {
        let html =
        `<div>
        真实用时: <div id='totalEffectiveTime'></div><br>
        运行时间: <div id='totalPlaytime'></div><br>
        <span class='show-when-time-borrowed'>${_txt("menu>options>time_borrowed")} <div id='borrowedTimeBalance'></div><br></span>
        循环数: <div id='totalLoops'></div><br>
        行动数: <div id='totalActions'></div><br>
        </div>`;
        return html;
    },
    prestige_bonuses() {
